...
class DVD extends Product {
    ...
    public function save($list) {
        if (!array_key_exists('size', $list)) {
            return null;
        }

        $this->size = filter_var($list['size'], FILTER_SANITIZE_NUMBER_FLOAT, FILTER_FLAG_ALLOW_FRACTION);

        if ($this->getSize() === false || $this->getSize() < 0) {
            return null;
        }
        ...
        try {
            ...
            $stmt->bind_param("ssis", $this->sku, $this->name, $this->price, DVD::$type);
            $stmt->execute();

            $query = "INSERT INTO product_attributes (SKU, ATTRIBUTE, VALUE) VALUES (?, ?, ?)";
            $stmt = $this->conn->prepare($query);
            $stmt->bind_param("ssi", $this->sku, DVD::$attribute, $this->size);
            $stmt->execute();
